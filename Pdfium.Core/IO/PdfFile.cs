...
{
    internal class PdfFile : IPdfFile, IDisposable
    {
        ...
        /// <summary>
        /// <see cref="IPdfFile.DocumentPointer"/>
        /// </summary>
        public IntPtr DocumentPointer => _document;

        /// <summary>
        /// <see cref="IPdfFile.FormPointer"/>
        /// </summary>
        public IntPtr FormPointer => _form;

        /// <summary>
        /// <see cref="IPdfFile.Bookmarks"/>
        /// </summary>
        public PdfBookmarkCollection Bookmarks { get; private set; }

        public PdfPageLinks GetPageLinks(int pageNumber, FloatSize pageSize)
        {
            if (_disposed)
                ...
                while (NativeMethods.FPDFLink_Enumerate(pageData.Page, ref link, out annotation))
                {
                    ...
                    if (NativeMethods.FPDFLink_GetAnnotRect(annotation, rect) && (target.HasValue || uri != null))
                    {
                        links.Add(new PdfPageLink(
                            new FloatRect(rect.left, rect.top, rect.right, rect.bottom),
                            target,
                            uri
                        ));
                    }
        ...
        public List<FloatSize> GetPDFDocInfo()
        {
            if (_disposed)
                throw new ObjectDisposedException(GetType().Name);

            int pageCount = NativeMethods.FPDF_GetPageCount(_document);
            var result = new List<FloatSize>(pageCount);
        ...
        public FloatSize GetPDFDocInfo(int pageNumber)
        {
            double height;
            double width;
                ...
                using (var pageData = new PageData(_document, _form, page))
                {
                    ...
                    try
                    {
                        while (NativeMethods.FPDFText_FindNext(handle))
                        {
                            ...
                            var textSpan = new PdfTextSpan(page, index, matchLength);

                            var pdfMatch = new PdfMatch(
                                match,
                                textSpan,
                                page
                            );
                            ...
                            if (readBounds)
                            {
                                ...
                                var textBounds = GetTextBounds(textSpan);
                                pdfMatch.TextBounds = textBounds?.ToArray();
                            }

                            matches.Add(pdfMatch);
                        }
                    }
                    finally
                    {
        ...
        private FloatRect GetBounds(IntPtr textPage, int index)
        {
