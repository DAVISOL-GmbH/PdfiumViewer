...
using Davisol.Pdfium.Windows.Render;
using System;
...
namespace Davisol.Pdfium.Windows.Print
{
    internal class PdfPrintDocument : PrintDocument
    {
        private readonly PdfDocumentRenderer _documentRenderer;
        ...
        internal PdfPrintDocument(PdfDocumentRenderer documentRenderer, PdfPrintSettings settings)
        {
            if (documentRenderer == null)
                throw new ArgumentNullException(nameof(documentRenderer));

            _documentRenderer = documentRenderer;
            _document = documentRenderer.Document;
            _settings = settings;
        }
        ...
        protected override void OnQueryPageSettings(QueryPageSettingsEventArgs e)
        {
            ...
            {
                bool landscape = GetOrientation(_document.PageSizes[_currentPage].ToSizeF()) == PdfPageOrientation.Landscape;
            ...
            if (_currentPage < pageCount)
            {
                ...
                for (int horizontal = 0; horizontal < settings.Horizontal; horizontal++)
                {
                    for (int vertical = 0; vertical < settings.Vertical; vertical++)
                    {
                        ...
                        if (settings.Orientation == PdfPageOrientation.Landscape)
        ...
        private void PrintSinglePage(PrintPageEventArgs e)
        {
            ...
            {
                var pageOrientation = GetOrientation(_document.PageSizes[_currentPage].ToSizeF());
                var printOrientation = GetOrientation(e.PageBounds.Size);

                e.PageSettings.Landscape = pageOrientation == PdfPageOrientation.Landscape;
        ...
        private PdfPageOrientation GetOrientation(SizeF pageSize)
        {
